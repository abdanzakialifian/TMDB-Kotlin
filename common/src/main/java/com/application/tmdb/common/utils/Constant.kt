package com.application.tmdb.common.utils

object Constant {
    const val FRAGMENT_DETAIL_DESTINATION = "fragment_detail"
    const val FRAGMENT_MOVIE_TV_SHOW_DESTINATION = "fragment_movie_tv_show"
    const val FRAGMENT_SPLASH_SCREEN_DESTINATION = "fragment_splash_screen"
    const val FRAGMENT_DETAIL_CAST_CREW_DESTINATION = "fragment_detail_cast_crew"
    const val KEY_ID = "id"
    const val KEY_INTENT_FROM = "intent_from"
    const val KEY_MOVIE = "movie"
    const val KEY_TV_SHOW = "tv_show"
    const val FORMAT_DD_MMM_YYYY = "dd MMM yyyy"
    const val FORMAT_YYYY_MM_DD = "yyyy-MM-dd"
    const val FORMAT_ICON_BULLET = "●"
}